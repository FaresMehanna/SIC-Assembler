...
static int cmp(void* f1 , void* f2){
	...
	if(strlen(*(char**)f1) != strlen(*(char**)f2))
		return 0;
	int len = strlen(*(char**)f1);

	for(int i=0;i<len;i++)
		if((*(char**)f1)[i] != (*(char**)f2)[i])
			return 0;
	return 1;
}
...
unsigned int strHash(void* key, int keysize, long long max){
...
int main (void){
	...
	HashSetChain x;
	InitializeHashSetChain(&x,sizeof(long long),sizeof(char*),cmp,NULL,freefn,strHash);
	int n = 1000;
	long long xzzy = 8;
	for(int i=0;i<n;i++){
		char* y = strdup("AAx");
		HashSetChainAddKey(&x,&y,&xzzy);
		printf("%lld\n", x.size);
		printf("%lld\n", x.actualsize);
	}
	printf("\n");
	...
	for(int i=0;i<n;i++){
		char* y = strdup("AAx");
		printf("%d",HashSetChainSearch(&x,&y,&xzzy));
		free(y);
	}
	printf("\n");
	printf("\n");
	...
	for(int i=0;i<n;i++){
		char* y = strdup("AAx");
		printf("%d",HashSetChainDelete(&x,&y));
		free(y);
	}

	DisposeHashSetChain(&x);
}
*/